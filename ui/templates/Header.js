...
export function Navigation({ name, mode = "dark" }) {
  return (
    <nav aria-label={name} className="sm:flex items-center hidden">
...
export default function Header({
  ...
  return (
    <>
      <header
        className={`h-[0] relative  z-40 ${className} ${
      ...
      >
        ...
        <div
          className={`px-10 ${
            mode == "dark" ? " bg-gradient-to-b from-black to-black/0" : ""
          }`}
        >
            ...
            <ul className="flex flex-col text-lg p-2">
              {mainMenu.map((item, key) => (
                <li key={key}>
                  <Link
                    href={item.href}
                    className={` px-4 py-2 rounded flex min-h-[3rem] items-center hover:bg-black hover:text-white text-black`}
                  >
                    {item.label}
                  </Link>
                </li>
